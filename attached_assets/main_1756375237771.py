...
async def goal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_db(db)
    reply = await ai_say(
        "Ты дружелюбный, но требовательный лайф-коуч. Подбодри и предложи следующий маленький шаг.",
...
async def plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply = await ai_say("Краткий, энергичный коуч.", prompt)
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_db(db)
    reply = await ai_say(
        "Ты коуч, который подводит итоги, отмечает успехи и предлагает 1 улучшение на завтра.",
